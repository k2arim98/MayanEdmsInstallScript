...
echo -n "* Changing the existing password..."
docker exec -it --user mayan mayan-edms \
/opt/mayan-edms/bin/mayan-edms.py shell \
 -c "from django.contrib.auth.models import User ; u = User.objects.get(username__exact='admin');u.set_password('${MAYAN_PASSWORD}');u.save()" >/dev/null
echo "Done"
...
TOKEN=$(curl -X POST http://localhost/api/v4/auth/token/obtain/?format=json  -H "Content-Type: application/json" -d '{"username":"admin","password":"password"}'  --user "admin:password" | jq -r '.token')

echo -n "* Adding document types..."
DocType=$(cat localhost.json | jq )
for i in $(jq -r ".label" localhost.json);   do   curl -X POST http://localhost/api/v4/document_types/ -H "Accept: application/json" -H "Authorization: token ${TOKEN}" -H "Content-Type: application/json" -d "$DocType" | jq ;  done
echo "Done"

echo -n "* Adding Cabinets..."
cabine=$(cat cabinet.json | jq )
for i in $(jq -r ".[] | i | .label" cabinet.json);   do   curl -X POST http://localhost/api/v4/cabinets/ -H "Accept: application/json" -H "Authorization: token ${TOKEN}" -H "Content-Type: application/json" -d "$cabine" | jq ;  done
echo "Done"
